...
#define beta sqrt(3.0f / 4.0f) * GyroMeasError * 0.3   // compute beta
// #define beta 0.041f 
#define beta_1 sqrt(3.0f / 4.0f) * GyroMeasError * 16
...
float q[4] = {0.0f, 1.0f, 0.0f, 0.0f};    // vector to hold quaternion
...
{  
 ...
 //   }
  ...
  MadgwickQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f, mx, my, mz, time);
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
    ...
    // }
    // if(time>=30){
    //   Serial.print("Yaw(DEG), Pitch(DEG), Roll(DEG), average_rate(Hz), time(sec) ");
    //   Serial.print(yaw, 2);
    //   Serial.print(", ");
    //   Serial.print(pitch, 2);
    //   Serial.print(", ");
...
// but is much less computationally intensive---it can be performed on a 3.3 V Pro Mini operating at 8 MHz!
        void MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz, float Time)
        {
            ...
            if(Time >2){
              ...
              qDot1 = 0.5f * (-q2 * gx - q3 * gy - q4 * gz) - beta_1 * s1;
              qDot2 = 0.5f * (q1 * gx + q3 * gz - q4 * gy) - beta_1 * s2;
              qDot3 = 0.5f * (q1 * gy - q2 * gz + q4 * gx) - beta_1 * s3;
              qDot4 = 0.5f * (q1 * gz + q2 * gy - q3 * gx) - beta_1 * s4;
            }
