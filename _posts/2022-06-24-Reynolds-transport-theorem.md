...
$\displaystyle \frac{DB_{sys}}{Dt}=\frac{\partial}{\partial t}\int_{\text{CV}}\rho b \, d\bar{V}\, + \,\int_{\text{CS}}\rho b \, \mathrm{\mathbf{V \cdot \hat{n}}} \, d A$가 성립한다.
...
미소 시간 $\delta t$동안 유체의 부피 변화 $\delta \bar{V} =\delta l_n \delta A = \delta l cos \theta \delta A$이고,

$B=mb$이므로 $\delta B = \delta(mb) = b \rho \delta \bar{V} \,=b \rho (V cos \theta \delta t)\delta A$이다.

이를 활용하면 $B$의 outflow rate $\delta \dot{B}_{\text{out}}$을 다음과 같이 나타낼 수 있다.

$\displaystyle \delta \overset{\cdot}B_{\text{out}} = \underset{\delta t \to 0}\lim\frac{\rho b \delta \bar{V}}{\delta t} = \underset{\delta t \to 0}\lim\frac{\rho b V cos \theta \delta t \delta A}{\delta t} = \rho b V cos \theta \delta A$
...
$\frac{DB_{\text{sys}}}{Dt} = \frac{\partial B_{\text{sys}}}{\partial t}+\overset{\cdot}B_{\text{out}}-\overset{\cdot}B_{\text{in}} =\frac{\partial B_{\text{CV}}}{\partial t}+\int_{CS}\rho b \, \mathrm{\mathbf{V \cdot \hat{n}}}\, d A=\frac{\partial}{\partial t}\int_{\text{CV}}\rho b \, d\bar{V}+\int_{CS}\rho b \, \mathrm{\mathbf{V \cdot \hat{n}}}$를 얻을 수 있다.
...
질량 보존 법칙에 의해 $\frac{DM_{\text{sys}}}{Dt}=0$이므로 $\frac{\partial}{\partial t}\int_{\text{CV}}\rho \, d\bar{V}\, + \,\int_{\text{CS}}\rho \, \mathrm{\mathbf{V \cdot \hat{n}}} \, d A=0$인데, steady flow를 가정하면 local derivative가 0이므로 뒤의 항만 남는다.
...
<div markdown="1">
$\displaystyle \sum F_\text{sys}=\frac{\partial}{\partial t}\int_{\text{CV}}V \rho \, d\bar{V}\, + \,\int_{\text{CS}}V \rho \, \mathrm{\mathbf{V \cdot \hat{n}}}$
...
$\displaystyle \sum (\mathrm{r \times F})\_{\text{sys}}=\frac{\partial}{\partial t}\int_{\text{CV}}\mathrm{r \times V} \rho \, d\bar{V}\, + \,\int_{\text{CS}}\mathrm{r \times V} \rho \, \mathrm{\mathbf{V \cdot \hat{n}}}$
...
$\displaystyle \left(\overset{\cdot}Q_\text{net, in}+\overset{\cdot}W_\text{net, in}\right)\_\text{CV} = \frac{\partial}{\partial t}\int_{\text{CV}}e \rho \, d\bar{V}\, + \,\int_{\text{CS}}e \rho \, \mathrm{\mathbf{V \cdot \hat{n}}}$

이때 $e$는 단위질량당 에너지로서, $e=\bar{u}+\frac{\mathrm{V^2}}{2}+gz$로 나타낼 수 있다.
...
$\frac{DM_{\text{sys}}}{Dt}=\frac{\partial}{\partial t}\int_{\text{CV}}\rho \, d\bar{V}\, + \,\int_{\text{CS}}\rho \, \mathrm{\mathbf{V \cdot \hat{n}}} \, d A=0$이다.

$\int_\text{CV}\rho d \bar{V} = \rho \left(lA+\bar{V}_\text{needle}\right)$임을 알 수 있다. 이때 $l$은 주사기 내부 공간의 길이를 의미하고, 피스톤이 움직임에 따라 $l$이 변하게 된다. 이것의 시간에 대한 변화율을 구해보면, $\bar{V}_\text{needle}$은 일정하므로 $\frac{\partial}{\partial t}\int_\text{CV}\rho d \bar{V} = \rho A \frac{\partial l}{\partial t}$이다. 피스톤을 미는 속력을 $V$라 하면, $\frac{\partial l}{\partial t} = -V$이다.(부호에 주의: $l$은 점점 줄어든다)
...
따라서 $\frac{DM_{\text{sys}}}{Dt}=\frac{\partial}{\partial t}\int_{\text{CV}}\rho \, d\bar{V}\, + \,\int_{\text{CS}}\rho \, \mathrm{\mathbf{V \cdot \hat{n}}} \, d A=-\rho A V + 1.1\rho Q=0, \,\, V=\frac{1.1Q}{A}$이다.
...
$\sum (\mathrm{r \times F})\_{\text{sys}}=\frac{\partial}{\partial t}\int_{\text{CV}}\mathrm{r \times V} \rho \, d\bar{V}\, + \,\int_{\text{CS}}\mathrm{r \times V} \rho \, \mathrm{\mathbf{V \cdot \hat{n}}}$
...
$\left(\frac{\partial}{\partial t}\int_{\text{CV}}\mathrm{r \times V} \rho \, d\bar{V}=0\right)$
